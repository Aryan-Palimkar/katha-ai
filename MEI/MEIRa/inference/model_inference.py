...
class Inference:
    def __init__(self, model_path):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(self.doc_encoder_path)
    ...
    def _load_model(self):
        config_path = path.join(self.model_path, "config.json")
        with open(config_path) as f:
            config_json = json.load(f)
        self.config = OmegaConf.create(config_json)

        # The model from hf is just the document encoder
        self.doc_encoder_path = path.join(self.model_path, "best", "doc_encoder")
        self.config.model.doc_encoder.transformer.model_str = self.doc_encoder_path

        self.model = EntityRankingModel(self.config.model, self.config.trainer)

        checkpoint_path = path.join(self.model_path, "best/model.pth")
    ...
    def perform_coreference(self, document, doc_name):
        ...
        for i, name in enumerate(ent_names):
            if i < len(clusters):
                mention_texts = [mention[1] for mention in clusters[i]]
                output_dict[name] = mention_texts
            else:
                output_dict[name] = []
...
if __name__ == "__main__":
    ...
    if path.isfile(doc_str):
        doc_name = path.basename(doc_str)
        with open(doc_str, encoding="utf-8") as f:
            doc_str = f.read()
    else:
        doc_name = "command_line"

    clusters = model.perform_coreference(doc_str, doc_name)
